...
// ONE FUNCTION SCOPE FOR THE WHOLE SHIM. The prelude is prepended as text in front of the module that docks it, so a
// top-level helper here would collide with a same-named task of that module (`setTitle` against the dom's
// `set-title`, which the emitter also spells `setTitle`) whenever the build keeps both, as the per-module dev build
// does. Scoping the helpers leaves only `html` at the top level.
function htmlNamespace() {
...
export const html = htmlNamespace()
