...
import type { Program, Statement, Type } from '@cluesurf/make/code/compile/node'
...
function typeKey(type: Type | undefined): string {
  ...
  switch (type.kind) {
    ...
    case 'named':
      return `${type.name}${type.args && type.args.length ? `<${type.args.map(typeKey).join(',')}>` : ''}`
    case 'function':
...
function signatureOf(statement: Statement): string | undefined {
  switch (statement.form) {
    case 'function': {
      ...
      const params = statement.params.map(p => `${p.name}:${typeKey(p.type)}`).join(',')
      const generics = statement.generics.map(g => `${g.name}${g.need ? `:${g.need}` : ''}`).join(',')
      return `fn ${statement.name}<${generics}>(${params})->${typeKey(statement.result)}${statement.async ? ' async' : ''}`
    }
    case 'record-type': {
      const fields = statement.fields.map(f => `${f.name}:${typeKey(f.type)}`).join(',')
      const variants = statement.variants
        .map(v => `${v.name}(${v.fields.map(f => `${f.name}:${typeKey(f.type)}`).join(',')})`)
        .join('|')
