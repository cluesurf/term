...
function main(): void {
  ...
  const straight = buildCfg(bodyOf(`task f\n  take n\n  save a, mark 1\n  back a\n`))
  ok('straight-line has a return terminator', straight.blocks.some((b) => b.terminator.kind === 'return'))
  ok('straight-line has no branch', !straight.blocks.some((b) => b.terminator.kind === 'branch'))

  // an if produces a branch with two distinct successors
  const branch = buildCfg(bodyOf(`task f\n  take n\n  fork test\n    hook test\n      call is-above\n        loan n\n        mark 0\n    hook hold\n      back n\n    hook miss\n      back, mark 0\n`))
  const branchBlock = branch.blocks.find((b) => b.terminator.kind === 'branch')
  ok('an if produces a branch terminator', branchBlock !== undefined)
  ok('the branch has two successors', branchBlock !== undefined && successors(branchBlock.terminator).length === 2)

  // a while loop has a back edge: the body returns to the header
  const loop = buildCfg(bodyOf(`task f\n  take n\n  walk test\n    hook test\n      call is-above\n        loan n\n        mark 0\n    hook step\n      save n\n        call subtract\n          loan n\n          mark 1\n  back n\n`))
  const header = loop.blocks.find((b) => b.terminator.kind === 'branch')
  let backEdge = false
  if (header) {
    for (const b of loop.blocks) if (successors(b.terminator).includes(header.id) && b.id !== header.id) backEdge = true
  }
