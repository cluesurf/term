...
function monoKey(powers: Map<string, number>): string {
  ...
  for (const [v, p] of [...powers].sort((a, b) => (a[0] < b[0] ? -1 : 1)))
...
function toPoly(expr: Expression): Poly | null {
  switch (expr.form) {
    case 'integer': {
      ...
      return Number.isInteger(n) ? new Map([['', n]]) : null
    }
    case 'variable':
...
// other provers.
export function ringEqual(left: Expression, right: Expression): boolean {
  const l = toPoly(left)
