...
// prepended (PKCS8 for private, X.509 for public). Reached only through the public key-agreement API.
object x25519 {
    ...
    fun sharedSecret(privateKey: ByteArray, publicKey: ByteArray): ByteArray {
        val priv = KeyFactory.getInstance("X25519").generatePrivate(PKCS8EncodedKeySpec(fromHex(PKCS8_PREFIX) + privateKey))
        val pub = KeyFactory.getInstance("X25519").generatePublic(X509EncodedKeySpec(fromHex(SPKI_PREFIX) + publicKey))
