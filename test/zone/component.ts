...
async function run(typescript: string): Promise<any> {
  const js = (await transform(typescript, { loader: 'ts', format: 'esm' })).code
  const file = path.join(os.tmpdir(), `seed-zone-${process.pid}-${pass + fail}.mjs`)
...
async function main(): Promise<void> {
  const result = compile({ file: 'zone.tree', text: SOURCE }, { resolve })
  ok('a zone compiles through resolve + infer + emit', result.ok, result.ok ? '' : JSON.stringify(result.diagnostics.slice(0, 4)))
  if (!result.ok) { console.log(`\nzone: ${pass} pass, ${fail} fail`); process.exit(1) }

  ok('emits a component function', /export function label/.test(result.typescript), '')
  ok('emits a positional element call', /element\("div"\)/.test(result.typescript))
  ok('emits a reactive dynamic node', /dynamic\(\(\) =>/.test(result.typescript))
  ...
  try {
    ...
    M.label(host, signal)
    const mounted = () => host.handle?.children?.[0]?.handle?.children?.[0]?.handle?.text
    ok('the zone mounts a reactive text node', mounted() === 'hello', String(mounted()))
    M.writeSignal(signal, 'world')
    ok('writing the signal updates the mounted node (reactive)', mounted() === 'world', String(mounted()))
    ...
    const attrs: Array<{ name: string; value: string }> = div?.handle?.attributes ?? []
    ok('an attribute is set on the element', attrs.some(a => a.name === 'role' && a.value === 'box'), JSON.stringify(attrs))
  } catch (e) {
    ok('the zone runs', false, String((e as Error).message))
    console.log('--- emitted ---'); console.log(result.typescript.split('\n').filter(l => /label|element|dynamic|append/.test(l)).join('\n'))
  } finally {
    fs.rmSync(file, { force: true })
  }

  // Stage B: fork -> show, walk -> each
  const control = compile({ file: 'control.tree', text: SOURCE_CONTROL }, { resolve })
  ok('a control-flow zone compiles', control.ok, control.ok ? '' : JSON.stringify(control.diagnostics.slice(0, 4)))
  if (control.ok) {
    ...
    try {
      ...
      ok('show renders the active branch', found.includes('ready') && !found.includes('waiting'), found.join(','))
      ok('each renders one node per item', ['a', 'b', 'c'].every(t => found.includes(t)), found.join(','))
    } catch (e) {
      ok('the control-flow zone runs', false, String((e as Error).message))
    }
  }
