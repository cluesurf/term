...
const lam = (param: string, body: Term): Term => ({ t: 'lam', param, body })
...
function main(): void {
  ...
  ok('before reduction there are three con nodes (two lam/app + inner app)', agentCount(lowered) === 3)

  lowered.net.normalize()
  ok('reduces to a single application node (f y)', agentCount(lowered) === 1)
  ...
  const survivor = [...lowered.net.nodes.keys()].find((id) => !lowered.net.interface.has(id))!
  ...
  ok('the application calls f', fnPeer.node === lowered.free.get('f')!.node)
  ok('the application is applied to y', argPeer.node === lowered.free.get('y')!.node)
