...
function program(text: string) {
  ...
  if (!parsed.ok) throw new Error('parse failed: ' + JSON.stringify(parsed.diagnostics))
  const built = mill(parsed.tree, 't.tree')
  if (!built.ok) throw new Error('mill failed: ' + JSON.stringify(built.diagnostics))
...
function main(): void {
  // L003: redundant arithmetic, with a verbatim-slice fix
  {
    ...
    ok('L003 detects x + 0', fs.length === 1, JSON.stringify(findings(text)))
    ok('L003 fix is the surviving operand', fs[0]?.fix?.text === 'read x', JSON.stringify(fs[0]?.fix))
  }
  ...
  // L004: a never-reassigned `save` should be `host`
  {
    ...
    ok('L004 flags never-reassigned save', fs.length === 1, JSON.stringify(findings(text)))
    ok('L004 fix swaps save -> host', fs[0]?.fix?.text === 'host', JSON.stringify(fs[0]?.fix))
    ...
    ok('L004 still fires when only rebound', findings(withAssign).filter((f) => f.code === 'L004').length >= 1, withAssign)
    void reassigned
  }
  ...
  // L001: non-kebab declared name
  {
    ...
    ok('L001 flags camelCase task name', fs.length === 1, JSON.stringify(findings(text)))
  }
  ...
  // analyze: one parse drives format + lint; inline suppression silences a rule
  {
    ...
    ok('analyze formats from the same parse', analysis.format().length > 0)
    ok('analyze suppresses L003 via comment', analysis.lint().filter((f) => f.code === 'L003').length === 0, JSON.stringify(analysis.lint()))
  }
