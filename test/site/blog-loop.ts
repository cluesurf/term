...
async function main(): Promise<void> {
  ...
  ok('userland has no driver imports', !/from ['"]pg['"]/.test(result.typescript) && !/from ['"]hono['"]/.test(result.typescript))
  ...
  const bundled = await build({
    ...
    external: ['pg', 'hono', '@hono/node-server'], write: false,
  })
  ...
  try {
    ...
    ok('connects to Postgres + starts hono', false, String((e as Error).message))
  ...
  const get = async (p: string): Promise<{ status: number; json: unknown }> => {
    const r = await fetch(`http://localhost:${PORT}${p}`)
    return { status: r.status, json: JSON.parse(await r.text()) }
  }

  const list = await get('/posts')
  const posts = list.json as Array<{ id: string; title: string; body: string }>
  ok('GET /posts returns the two posts as JSON', list.status === 200 && posts.length === 2, JSON.stringify(list.json))
  ok('first post came through the loop (Postgres -> trie -> hono -> fetch)',
    posts[0]?.id === 'p1' && posts[0]?.title === 'First Post' && posts[0]?.body === 'Hello world', JSON.stringify(posts[0]))
  ok('second post too', posts[1]?.id === 'p2' && posts[1]?.title === 'Second Post', JSON.stringify(posts[1]))

  const one = await get('/posts/p2')
  const single = one.json as { id: string; title: string; body: string }
  ok('GET /posts/:id binds the id and reads one post', one.status === 200 && single.id === 'p2' && single.title === 'Second Post' && single.body === 'More text', JSON.stringify(one.json))
