...
export function formatTable(results: Array<BenchmarkResult>): string {
  ...
  const header = `  ${pad('benchmark', nameWidth)}  ${padLeft('mean', 10)}  ${padLeft('median', 10)}  ${padLeft('ops/sec', 14)}  ${padLeft('cv', 7)}`
  const line = `  ${'-'.repeat(nameWidth)}  ${'-'.repeat(10)}  ${'-'.repeat(10)}  ${'-'.repeat(14)}  ${'-'.repeat(7)}`
  const body = rows.map((r) => `  ${pad(r.name, nameWidth)}  ${padLeft(r.mean, 10)}  ${padLeft(r.median, 10)}  ${padLeft(r.ops, 14)}  ${padLeft(r.cv, 7)}`)
