...
import { ringEqual } from '@/code/check/ring'

type Fn = Extract<Statement, { form: 'function' }>
type Binary = Extract<Expression, { form: 'binary' }>

const intE = (value: number, span: Span): Expression => ({ form: 'integer', value, span })
const varE = (name: string, span: Span): Expression => ({ form: 'variable', name, span })
const binE = (
  op: Binary['op'],
  left: Expression,
  right: Expression,
...
function subst(e: Expression, name: string, repl: Expression): Expression {
  switch (e.form) {
    case 'variable':
      return e.name === name ? repl : e
    case 'binary':
      return { ...e, left: subst(e.left, name, repl), right: subst(e.right, name, repl) }
    case 'unary':
    ...
    case 'conditional':
      return {
        ...
        otherwise: e.otherwise ? subst(e.otherwise, name, repl) : undefined,
      }
    default:
      return e
  }
  ...
  switch (e.form) {
    case 'call': {
      ...
      if (
        ...
        return build(args[0]!)
      return { ...e, callee: replaceCall(e.callee, fname, match, build), args }
    }
    case 'binary':
      return {
        ...e,
    ...
    case 'unary':
      return { ...e, operand: replaceCall(e.operand, fname, match, build) }
    case 'conditional':
      return {
        ...e,
        branches: e.branches.map(b => ({
...
function recurrence(
  ...
  const ret = fn.body[0]!
  if (ret.form !== 'return' || !ret.value || ret.value.form !== 'conditional')
    return null
...
export function checkFold(
  ...
  if (base.form !== 'binary' || !ringEqual(base.left, base.right)) return false
  ...
  const matchK = (a: Expression): boolean => ringEqual(a, varE(inductVar, span))
  const stepGoal = replaceCall(expanded, fname, matchK, () => S) as Binary
