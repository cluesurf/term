...
export function parseMarkHold(text) {
    // union: "0.14.x|0.15.x"
    if (text.includes('|')) {
...
export function compareMark(a, b) {
    if (a.major !== b.major)
...
export function markMatch(mark, hold) {
    switch (hold.form) {
        case 'exact':
            return compareMark(mark, hold.mark) === 0;
        case 'wild':
            if (mark.major !== hold.major)
                return false;
            ...
            return true;
        case 'band':
            return (compareMark(mark, hold.base) >= 0 &&
