...
ok('compiles in per-module mode', result.ok, result.ok ? '' : JSON.stringify(result.diagnostics?.slice(0, 3)))
if (!result.ok) { console.log(`\nmodules: ${pass} pass, ${fail} fail`); process.exit(1) }

const modules = result.modules!
ok('emits one module per source file', modules.size === 2, `size ${modules.size}`)

const entryCode = modules.get('entry.tree')?.code ?? ''
const helperCode = modules.get('helper.tree')?.code ?? ''

ok('entry records its dependency edge on the helper', modules.get('entry.tree')?.imports.includes('helper.tree') === true, JSON.stringify(modules.get('entry.tree')?.imports))

ok('entry imports the cross-module function (greeting)', /import \{[^}]*\bgreeting\b/.test(entryCode), entryCode)
ok('entry imports the cross-module function (originPoint)', /import \{[^}]*\boriginPoint\b/.test(entryCode), entryCode)
ok('entry imports the cross-module type (Point)', /import type \{[^}]*\bPoint\b/.test(entryCode), entryCode)
ok('entry imports from the helper url', entryCode.includes(`"${urlForFile('helper.tree')}"`), entryCode)
ok('helper exports the function', /export (async )?function greeting/.test(helperCode), helperCode)
ok('helper has no cross-module import (it defines everything it uses)', !/^import /m.test(helperCode), helperCode)
...
const M = (await import(pathToFileURL(path.join(dir, `${safe('entry.tree')}.mjs`)).href)) as {
  run: () => string
  startPoint: () => { x: number; y: number }
}
ok('the wired ESM modules run: run() returns the helper value', M.run() === 'hi', JSON.stringify(M.run()))
ok('the wired ESM modules run: startPoint() builds the cross-module form', M.startPoint().x === 0 && M.startPoint().y === 0, JSON.stringify(M.startPoint()))
...
ok('blog compiles in per-module mode', blog.ok, blog.ok ? '' : JSON.stringify(blog.diagnostics?.slice(0, 3)))
if (blog.ok && blog.modules) {
  const blogModules = blog.modules
  ok('blog emits many modules (full stdlib + framework graph)', blogModules.size > 5, `size ${blogModules.size}`)
  ...
  const postEntry = [...blogModules].find(([f]) => f.endsWith('back/post.tree'))
  const postCode = postEntry?.[1]?.code ?? ''
  ok('post repository imports db functions across modules',
    /import \{[^}]*\b(connect|query|run|field)\b/.test(postCode), postCode.slice(0, 240))

  // the API module imports the post repository's functions
  const apiEntry = [...blogModules].find(([f]) => f.endsWith('hook/blog.tree'))
  const apiCode = apiEntry?.[1]?.code ?? ''
  ok('api module imports the repository across modules',
    /import \{[^}]*\b(setup|listPosts|getPost|createPost)\b/.test(apiCode), apiCode.slice(0, 240))
}
