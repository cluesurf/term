...
async function main(): Promise<void> {
  ...
  const bundled = await build({
    ...
    external: ['hono', '@hono/node-server'], write: false,
  })
  ...
  try {
    M = (await import(file)) as unknown as { start: (port: number) => void }
    M.start(PORT)
  ...
  const fetchText = async (p: string): Promise<{ status: number; body: string }> => {
    const r = await fetch(`http://localhost:${PORT}${p}`)
    return { status: r.status, body: await r.text() }
  }

  const health = await fetchText('/health')
  ok('static /health dispatches (200 ok)', health.status === 200 && health.body === 'ok', JSON.stringify(health))

  const u42 = await fetchText('/users/42')
  ok(':param /users/42 binds id=42', u42.status === 200 && u42.body === '42', JSON.stringify(u42))

  const u7 = await fetchText('/users/7')
  ok(':param /users/7 binds id=7', u7.status === 200 && u7.body === '7', JSON.stringify(u7))

  const miss = await fetchText('/nope')
  ok('unmatched path returns 404', miss.status === 404, JSON.stringify(miss))
