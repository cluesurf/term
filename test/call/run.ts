...
writeFileSync(join(root, 'code', 'double.tree'), 'task double\n  take n, like number\n  like number\n  send back\n    call multiply\n      read n\n      mark 2\n')
...
expect('emitted code defines the function', existsSync(out) && readFileSync(out, 'utf8').includes('function double'), true)

// a broken file is reported, not written
writeFileSync(join(root, 'code', 'broken.tree'), 'task broken\n  send back\n    read missing-name\n')
...
expect('surfaces the diagnostic', mixed.errors.some((e) => e.includes('broken.tree')), true)
