...
ok(
  ...
  proves(
    [atMost(V('a'), V('b')), atMost(V('b'), V('c'))],
    atMost(V('a'), V('c')),
  ),
)
ok(
  'n>=1 => n>=0',
  proves([atLeast(V('n'), N(1))], atLeast(V('n'), N(0))),
)
ok(
  'strict n>0 => n>=1 (integer)',
  proves([above(V('n'), N(0))], atLeast(V('n'), N(1))),
)
ok(
  'a<=b => a+c<=b+c',
  proves(
    [atMost(V('a'), V('b'))],
    atMost(linear({ a: 1, c: 1 }), linear({ b: 1, c: 1 })),
  ),
)
ok(
  'chain x0..x8 => x0<=x8',
  proves(
    Array.from({ length: 8 }, (_, i) =>
      atMost(V('x' + i), V('x' + (i + 1))),
    ),
    atMost(V('x0'), V('x8')),
  ),
)
ok(
  '2a<=2b => a<=b (gcd reduce)',
  proves(
    [atMost(linear({ a: 2 }), linear({ b: 2 }))],
    atMost(V('a'), V('b')),
  ),
)
ok(
  'NOT: a<=b => b<=a',
  !proves([atMost(V('a'), V('b'))], atMost(V('b'), V('a'))),
)
ok(
  'NOT: n>=0 => n>=1',
  !proves([atLeast(V('n'), N(0))], atLeast(V('n'), N(1))),
)
ok(
  'NOT: a<=b,b<=c => a<c',
  !proves(
