...
function expectOk(name: string, source: string): void {
  ...
  if (result.ok) {
    ...
    fail++
    console.log(`FAIL  ${name}  (${result.diagnostics.map((d) => d.name).join(',')})`)
  }
}

function expectEffectError(name: string, source: string): void {
  const result = compile({ file: 'e.tree', text: source })
  if (!result.ok && result.diagnostics.some((d) => d.name === 'effect-error')) {
    pass++
    console.log(`ok    ${name}  (${result.diagnostics.find((d) => d.name === 'effect-error')!.message})`)
  } else {
    fail++
...
`,
  ...
  const ASYNC_CB = (callerAsync: boolean, await_: boolean): string => `task run-cb
  take f
    like task
      wait true
      take x, like number
  ...
  back
    call f
      loan n${await_ ? '\n      wait true' : ''}
`
  expectOk('async callback awaited inside an async task', ASYNC_CB(true, true))
  expectEffectError('async callback not awaited is rejected', ASYNC_CB(false, false))
  expectEffectError(
...
`
  ...
  const built = parsed.ok ? mill(parsed.tree, 'e.tree') : { ok: false as const, diagnostics: [] }
  if (built.ok) {
    const rows = effectRows(built.program)
    const ok2 = (name: string, cond: boolean): void => {
      if (cond) { pass++; console.log(`ok    ${name}`) } else { fail++; console.log(`FAIL  ${name}`) }
    }
    ok2('a function that busts has the throw effect', rows.get('boom')?.has('throw') === true)
    ok2('throw propagates to a caller (transitive row)', rows.get('relay')?.has('throw') === true)
    ok2('a non-throwing function has no throw effect', rows.get('quiet')?.has('throw') !== true)
  } else {
    fail++
...
`
    ...
    const built2 = parsed2.ok ? mill(parsed2.tree, 'e.tree') : { ok: false as const, diagnostics: [] }
    if (built2.ok) {
      const rows = effectRows(built2.program)
      const ok2 = (name: string, cond: boolean): void => {
        if (cond) { pass++; console.log(`ok    ${name}`) } else { fail++; console.log(`FAIL  ${name}`) }
      }
      ok2('a function calling a throwing callback inherits throw', rows.get('apply-it')?.has('throw') === true)
    } else {
      fail++
