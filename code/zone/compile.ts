...
export type ZoneResult = { ok: true; typescript: string } | { ok: false; diagnostics: Array<Diagnostic> }

export function compileZone(source: { file: string; text: string }): ZoneResult {
  ...
  const name = nameGroup && nameGroup.kind === 'group' ? head(nameGroup) : 'zone'
  ...
  for (const node of parts.slice(1)) {
    if (node.kind === 'group' && head(node) === 'state') {
      ...
      const sName = inner[0] && inner[0].kind === 'group' ? head(inner[0]) : undefined
      if (sName) {
  ...
  for (const node of parts.slice(1)) {
    if (node.kind === 'group' && head(node) === 'hook') {
      const inner = rest(node)
      const event = inner[0] && inner[0].kind === 'group' ? head(inner[0]) : undefined
      if (event) handlers.push({ event, body: inner.slice(1).map((s) => statement(s, stateNames)) })
    }
  }
...
function expr(node: Node | undefined, stateNames: Set<string>): string {
  ...
  switch (node.kind) {
    ...
    case 'decimal':
      return String(node.value)
    case 'text':
      return JSON.stringify(node.parts.map((p) => (p.kind === 'chunk' ? p.text : '')).join(''))
    case 'group': {
      const kw = head(node)
      ...
      if (kw === 'loan' || kw === 'read' || kw === 'move') {
        ...
        return stateNames.has(v ?? '') ? `${toCamel(v ?? '')}()` : toCamel(v ?? '')
      }
      if (kw === 'call') {
        const fn = args[0] && args[0].kind === 'group' ? head(args[0]) : undefined
        const callArgs = args.slice(1).map((a) => expr(a, stateNames))
        if (fn && fn in BINARY && callArgs.length === 2) return `${callArgs[0]} ${BINARY[fn]} ${callArgs[1]}`
        if (fn === 'decrement' && callArgs.length === 1) return `${callArgs[0]} - 1`
        if (fn === 'increment' && callArgs.length === 1) return `${callArgs[0]} + 1`
        return `${toCamel(fn ?? '')}(${callArgs.join(', ')})`
      }
      // a bare name: a variable or state read
      if (kw && args.length === 0) return stateNames.has(kw) ? `${toCamel(kw)}()` : toCamel(kw)
      return 'undefined'
    }
    default:
      return 'undefined'
...
function statement(node: Node, stateNames: Set<string>): string {
  ...
  if (kw === 'save') {
    const args = rest(node)
    const target = args[0] && args[0].kind === 'group' ? head(args[0]) : undefined
    if (target && stateNames.has(target)) {
      return `set${toPascal(target)}(${expr(args[1], stateNames)})`
    }
    return `let ${toCamel(target ?? 'x')} = ${expr(args[1], stateNames)}`
  }
...
function emit(
  ...
  const lines: Array<string> = [`import { signal } from '@/code/zone/reactive'`, '']
  lines.push(`export function ${toCamel(name)}() {`)
  for (const s of state) {
    lines.push(`  const [${toCamel(s.name)}, set${toPascal(s.name)}] = signal(${s.init})`)
  }
  ...
  const returned = [...state.map((s) => toCamel(s.name)), ...handlers.map((h) => `on${toPascal(h.event)}`)]
