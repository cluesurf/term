...
function symmetricPositiveSemidefinite(m: Array<Array<Rat>>): boolean {
  const n = m.length
  for (let i = 0; i < n; i++) {
    const pivot = m[i]![i]!
...
function quadraticFormNonNegative(d: Poly): boolean {
  ...
  const m: Array<Array<Rat>> = Array.from({ length: size }, () =>
  ...
  for (const [mono, coeff] of d) {
    ...
    if (powers.length === 0) put(0, 0, rat(BigInt(coeff))) // constant term
    else if (powers.length === 1) {
      const [v, p] = powers[0]!
      const i = indexOf(v) + 1
      if (p === 1) put(0, i, half(coeff)) // linear term: coeff/2 off the constant row
