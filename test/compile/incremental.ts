...
async function main(): Promise<void> {
  ...
  // ---- the signature firewall on real definitions ----
  {
    ...
    const checkAll = (): Promise<void> =>
      run(fw, async cx => {
        for (const n of await fw.defNames(cx, files)) await fw.checkDef(cx, files, n)
      })

    ok('finds the three definitions', (await run(fw, cx => fw.defNames(cx, files))).join(',') === 'caller,helper,other')
    await checkAll()
    ok('caller resolves its call to the defined helper', (await run(fw, cx => fw.checkDef(cx, files, 'caller'))).unresolved.length === 0)
  ...
  // ---- functional per-definition resolve (real resolve, firewalled) ----
  {
    ...
    const resolveAll = (): Promise<void> =>
      run(rc, async cx => {
        for (const n of await rc.defNames(cx, files)) await rc.resolvedDef(cx, files, n)
      })

    const caller = await run(rc, cx => rc.resolvedDef(cx, files, 'caller'))
    ...
    rc.setSource(X, withHelper(`task helper\n  take n, like number\n  like number\n  send back\n    call missing-fn\n      read n\n`))
    const broken = await run(rc, cx => rc.resolvedDef(cx, files, 'helper'))
    ...
    ok('resolvedDef: an unrelated function stays clean', (await run(rc, cx => rc.resolvedDef(cx, files, 'other'))).diagnostics.length === 0)
  }
  ...
  // ---- functional per-definition type-checking (real infer) ----
  {
    ...
    ok('typedDef: an unrelated function stays clean under the error', (await run(tc, cx => tc.typedDef(cx, files, 'other'))).diagnostics.length === 0)
  }
  ...
  // ---- per-definition emit (stage 3) ----
  {
    ...
    ok('emitDef: the caller emit is unchanged', (await run(ec, cx => ec.emitDef(cx, files, 'caller'))) === callerEmit)
  }
