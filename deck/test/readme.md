...
- `npx tsx deck/test/code/prove-cli.ts <file.tree>` - **the `seed hold` terminal workflow** (built, runnable): compiles a file, reports structured verification gaps, runs the cross-backend differential, and exits 0/1 to gate CI. `fixture/good.tree` -> PROVED, exit 0; `fixture/bad.tree` -> the gap, exit 1.
...
- `npx tsx deck/test/code/demo-seed-culmination.ts` (4/4) - **every synthesis strategy culminating in compiling Seed**: each of the four strategies synthesizes an expression, which is lowered via `emitSeed` and compiled by the LIVE Seed compiler. Spec/examples in, compiling Seed out, by every strategy in the field.
...
- `npx tsx deck/test/code/demo-ctl.ts` (6/6) - **CTL temporal logic over BDDs**: `EX/EF/EG/EU/AG/AF` as BDD fixpoints, checked symbolically against a system's state set.
- `npx tsx deck/test/code/demo-cegar.ts` (2/2) - **counterexample-guided abstraction refinement**: predicate abstraction, abstract reachability, spurious-counterexample detection, and refinement (the concrete BMC re-check that rules out a false abstract path).
- `npx tsx deck/test/code/demo-symmetry.ts` (3/3) - **symmetry reduction**: explores a state graph up to a canonicalizing symmetry, collapsing symmetric states to one representative.
- `npx tsx deck/test/code/demo-seed-check.ts` (2/2) - **model checking Seed itself**: encodes a real invariant from `@cluesurf/link` (the unattended loop's state machine) as a transition system and model-checks the safety property.
...
- **`synth-extra.ts`** - the rest of the synthesis-strategy field over the same grammar: `observationalEnum` (bottom-up enumeration with observational-equivalence pruning), `constraintSynth` (constraint-based exists-forall, the SyGuS single-query via Z3 `ForAll`), `stochasticSynth` (Metropolis-Hastings MCMC), and `fillSketch` (sketching - fill a program hole). Every strategy's output lowers to compiling Seed.
