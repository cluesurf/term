...
function main(): void {
  // n >= 0  implies  n + 1 > 0
  expect('n>=0 => n+1>0', proves([atLeast(n, k(0))], above(linear({ n: 1 }, 1), k(0))), true)
  ...
  expect('bounds => i>=0', proves([atLeast(i, k(0)), below(i, len)], atLeast(i, k(0))), true)

  // transitivity: a <= b AND b <= c  implies  a <= c
  const a = linear({ a: 1 })
  const b = linear({ b: 1 })
  const c = linear({ c: 1 })
  expect('a<=b, b<=c => a<=c', proves([atMost(a, b), atMost(b, c)], atMost(a, c)), true)
  ...
  expect('contradiction proves anything', proves([atLeast(n, k(5)), atMost(n, k(0))], atMost(n, k(-100))), true)
