...
import { startDevServer } from '@/code/dev/server'
import { applyHmr } from '@/code/dev/client'
import type { HmrMessage } from '@/code/dev/client'
...
async function readEvent(reader: ReadableStreamDefaultReader<Uint8Array>): Promise<HmrMessage | undefined> {
...
async function main(): Promise<void> {
  const server = startDevServer({ root: SEED, entry, port: PORT, env: 'browser' })
  await new Promise(r => setTimeout(r, 300))

  // 1. the app shell loads the client + the entry module
  const shell = await (await fetch(`http://localhost:${PORT}/`)).text()
  ok('serves the app shell', shell.includes('/@seed/client.mjs') && shell.includes('/@mod/'), shell.slice(0, 120))
  ...
  ok('serves a compiled module as javascript',
    modResponse.status === 200 && (modResponse.headers.get('content-type') ?? '').includes('javascript') && modCode.includes('function main'),
    `${modResponse.status} ${modResponse.headers.get('content-type')}`)

  // 3. the client runtime is served
  const client = await (await fetch(`http://localhost:${PORT}/@seed/client.mjs`)).text()
  ok('serves the hmr client runtime', client.includes('EventSource') && client.includes('applyHmr'))
  ...
  ok('sse sends connected on open', first?.type === 'connected', JSON.stringify(first))

  const result = server.update(entry) // the entry is a non-accepting root -> full reload
  ok('update of a non-accepting root decides full-reload', result.type === 'full-reload', JSON.stringify(result))
  const pushed = await readEvent(reader)
  ok('sse pushes the hmr message to the client', pushed?.type === 'full-reload', JSON.stringify(pushed))
  ...
  let reloaded = false
  await applyHmr({ type: 'full-reload' }, {
  ...
  await applyHmr(
    { type: 'update', updates: [{ boundary: '/z.mjs', accepted: '/z.mjs', timestamp: 7 }] },
    { reload: () => {}, reimport: async () => ({ fresh: true }), acceptOf: () => m => { accepted = m }, log: () => {} },
  )
  ok('client applyHmr: update re-imports and runs the accept callback', JSON.stringify(accepted) === JSON.stringify({ fresh: true }), JSON.stringify(accepted))
