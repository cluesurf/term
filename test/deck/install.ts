...
import { install } from '@/code/deck/install'
import { storePath } from '@/code/deck/resolve'
...
function writePackage(registry: string, host: string, name: string, version: string, deckText: string): void {
...
function main(): void {
  ...
  writePackage(registry, 'termsurf', 'b', '0.0.1', `deck @termsurf/b\n  mark <0.0.1>\n`)
  writePackage(
    registry,
    'termsurf',
    'a',
  ...
  writeFileSync(join(project, 'deck.tree'), `deck @my/app\n  mark <0.1.0>\n  link @termsurf/a\n    mark <*>\n`)
  ...
  expect('store has a', existsSync(storePath(storeHome, 'termsurf', 'a', '0.0.1')), true)
  expect('store has b (transitive)', existsSync(storePath(storeHome, 'termsurf', 'b', '0.0.1')), true)
  expect('store copied files', existsSync(join(storePath(storeHome, 'termsurf', 'a', '0.0.1'), 'code.tree')), true)
  ...
  expect('link resolves to store file', existsSync(join(linkA, 'code.tree')), true)

  // the lockfile records the resolved graph
  expect('lockfile requests a', result.lockfile.requests.some((r) => r.name === '@termsurf/a' && r.locked === '0.0.1'), true)
  expect('lockfile links a and b', result.lockfile.links.length, 2)
  const aLink = result.lockfile.links.find((l) => l.ref.startsWith('@termsurf/a'))
  expect('a depends on b in lockfile', aLink?.deps.some((d) => d.name === '@termsurf/b'), true)

  // the lockfile was written to disk
  expect('lockfile on disk', existsSync(join(project, 'deck.lock.tree')), true)
  expect('lockfile mentions b', readFileSync(join(project, 'deck.lock.tree'), 'utf8').includes('@termsurf/b'), true)
