...
import { bind, check, checks, contextWithSignature, defineConstant, emptyContext, eqLevel, evaluate, freshMeta, infer, instantiateLevel, litLevel, resetDefinitions, resetMetas, varLevel, TypeError as CoreTypeError } from '@/code/check/judge'
...
const sigma = (mult: Mult, domain: Term, codomain: Term): Term => ({ tag: 'sigma', mult, domain, codomain })
const pair = (first: Term, second: Term): Term => ({ tag: 'pair', first, second })
...
function main(): void {
  ...
  ok('metavariable solved by unification (polymorphic identity at a concrete type)', () => {
    resetMetas()
  ...
  rejects('metavariable solving rejects an inconsistent result type', () => {
    resetMetas()
    const context = contextWithSignature(signature)
    const meta = freshMeta(typeValue(0))
    const term = app(app(kconst('identity'), meta), kconst('value'))
  ...
  ok('a linear region is consumed exactly once', () => {
    ...
    check(context, lam(v(0)), evaluate([], pi(1, kconst('Region'), kconst('Region'))))
  })
  rejects('aliasing a linear region is rejected (used twice)', () => {
    ...
    check(context, lam(pair(v(0), v(0))), evaluate([], pi(1, kconst('Region'), sigma('many', kconst('Region'), kconst('Region')))))
  })
  rejects('dropping a linear region is rejected (used zero times)', () => {
    const context = contextWithSignature([...regionSig, { name: 'unit', type: ty(0) }, { name: 'nothing', type: kconst('unit') }])
    // \ r. nothing  :  (1 r : Region) -> unit  -- never uses the region, violating linearity
    check(context, lam(kconst('nothing')), evaluate([], pi(1, kconst('Region'), kconst('unit'))))
  })

  // ---- Miller pattern unification: a metavariable applied to distinct bound variables ----
  const millerSig = [{ name: 'Nat', type: ty(0) }, { name: 'value', type: kconst('Nat') }]
  const NatToNat = pi('many', kconst('Nat'), kconst('Nat'))
  ok('Miller pattern: ?m x = x solves ?m to the identity, then computes', () => {
    resetMetas()
    const context = bind(contextWithSignature(millerSig), 'many', evaluate([], kconst('Nat'))) // x : Nat at level 0
    ...
    check(context, refl(kconst('Nat'), v(0)), evaluate(context.env, goal))
    // the solution must compute: (?m value) reduces to value
    const computed = id(kconst('Nat'), app(m, kconst('value')), kconst('value'))
    check(context, refl(kconst('Nat'), kconst('value')), evaluate(context.env, computed))
  })
  rejects('a non-pattern spine (argument is not a distinct variable) does not solve', () => {
    resetMetas()
    ...
    const goal = id(kconst('Nat'), app(m, kconst('value')), kconst('value'))
    check(context, refl(kconst('Nat'), kconst('value')), evaluate([], goal))
  })

  // ---- transparent definitions (delta reduction) ----
  const deltaSig = [
    { name: 'Nat', type: ty(0) },
    { name: 'identityFn', type: pi('many', kconst('Nat'), kconst('Nat')) },
    { name: 'value', type: kconst('Nat') },
  ]
  ok('a transparent definition unfolds during conversion (delta)', () => {
    resetMetas()
    resetDefinitions()
    ...
    const goal = id(kconst('Nat'), app(kconst('identityFn'), kconst('value')), kconst('value'))
    check(context, refl(kconst('Nat'), kconst('value')), evaluate([], goal))
  })
  ...
  rejects('fuel-bounded delta terminates on a recursive / self-referential definition', () => {
    resetMetas()
    resetDefinitions()
    ...
    check(context, refl(kconst('Nat'), kconst('loopy')), evaluate([], goal))
  })
  ...
  ok('a pair checks against its sigma type', () => {
    ...
    check(context, pair(kconst('value'), kconst('value')), evaluate([], NatPair))
  })
  ok('projections fst/snd type-check', () => {
    const context = contextWithSignature(data)
    const annotated = ann(pair(kconst('value'), kconst('value')), NatPair)
  ...
  rejects('a pair with a wrong component is rejected', () => {
    const context = contextWithSignature([...data, { name: 'flag', type: kconst('Bool') }])
    check(context, pair(kconst('value'), kconst('flag')), evaluate([], NatPair)) // second must be Nat
  })

  // ---- observational equality beyond Pi: Id at a product is a product of Ids (pair extensionality computes) ----
  ok('pair extensionality computes (Id at a product reduces to a product of Ids)', () => {
    const context = contextWithSignature(data)
    const point = pair(kconst('value'), kconst('value'))
    const identity = id(NatPair, point, point) // Id ((Nat)*Nat) p p  ==>  (Id Nat ..) * (Id Nat ..)
    const proof = pair(refl(kconst('Nat'), kconst('value')), refl(kconst('Nat'), kconst('value')))
  ...
  ok('dependent pair extensionality computes (Id at a dependent sigma, via transport)', () => {
    const signature = [
      { name: 'Nat', type: ty(0) },
    ...
    const context = contextWithSignature(signature)
    const dependent = sigma('many', kconst('Nat'), app(kconst('Vec'), v(0))) // Sigma (x : Nat) Vec x
    ...
    const proof = pair(refl(kconst('Nat'), kconst('n')), refl(app(kconst('Vec'), kconst('n')), kconst('vn')))
    check(context, proof, evaluate([], identity))
  })

  // ---- self types (the inductive foundation) ----
  ok('self introduction: a value checks against a non-dependent self type', () => {
    const context = contextWithSignature(data)
    check(context, kconst('value'), evaluate([], self(kconst('Nat')))) // value : Self x. Nat  iff  value : Nat
  })
  ok('self elimination: a self-typed variable is usable at the unfolded type', () => {
    const context = bind(contextWithSignature(data), 'many', evaluate([], self(kconst('Nat'))))
  ...
  ok('an inductive (Bool) is derivable as a self-type encoding', () => {
    ...
    const boolEncoding = self(
      pi('many', pi('many', kconst('Bool'), ty(0)), // P : Bool -> Type0
        pi('many', app(v(0), kconst('true')), // P true
          pi('many', app(v(1), kconst('false')), // P false
            app(v(2), v(3))))), // P x
    )
    ...
    if (inferred.type.v !== 'type') throw new CoreTypeError('the Bool self-encoding is not a type')
  })
