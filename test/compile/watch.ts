...
const files = new Map<string, string>([
  ['@app/helper', 'task triple\n  take n, like number\n  like number\n  send back\n    call multiply\n      read n\n      mark 3\n'],
])
const resolve = (path: string): Source | undefined => (files.has(path) ? { file: path, text: files.get(path)! } : undefined)
const entry = (constant: number): Source => ({ file: 'entry.tree', text: `load @app/helper\n  find triple\n\ntask run\n  like number\n  send back\n    call triple\n      mark ${constant}\n` })
...
expect('unchanged recompile adds no misses', compiler.stats.misses, missesAfterFirst)
...
expect('the unchanged helper is reused (a mill hit)', compiler.stats.hits > hitsBeforeEdit, true)
expect('only the entry + the new graph key miss', compiler.stats.misses - missesBeforeEdit, 2)

// 4. edit the helper itself: now the helper re-mills (a miss), the entry text is unchanged so it is reused
files.set('@app/helper', 'task triple\n  take n, like number\n  like number\n  send back\n    call multiply\n      read n\n      mark 4\n')
...
expect('the unchanged entry is reused after a helper edit', compiler.stats.hits > hitsBeforeHelper, true)
