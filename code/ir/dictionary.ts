...
export function passDictionaries(program: Program): Program {
  ...
  for (const statement of program) {
    ...
    instanceMethods.set(`${statement.mask}:${statement.target}`, methods)
  }
  ...
  const boundsOf = new Map<string, Array<{ name: string; mask: string }>>()
  ...
  function rewriteTraitCalls(
    ...
    walkExpressions(body, node => {
      if (node.form !== 'call' || node.callee.form !== 'variable') return
      ...
      node.callee = {
        form: 'member',
        target: { form: 'variable', name: dict, span: node.callee.span },
  ...
  function rewriteCallSites(body: Array<Statement>, enclosing: Fn): void {
  ...
  function dictArgument(
    ...
    if (concrete && concrete.kind === 'named') {
      ...
      if (instanceMethods.has(key)) {
        neededDicts.add(key)
        return { form: 'variable', name: dictConst(mask, concrete.name), span }
      }
    }
