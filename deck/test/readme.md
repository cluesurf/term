...
- `npx tsx deck/test/code/demo-proof-lsp.ts` (5/5) - **the LSP proof-protocol server**: `proof/state` (the InfoView's goal list) and `proof/hammer` (close a goal) running end to end - open a document, render goals, hammer them, re-render (`[OK]`/`[BAD]`/`[...]`). The server half of the proof IDE; only the webview client needs an editor runtime.
