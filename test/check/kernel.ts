...
import { annotate, apply, checkBook, lambda, pi, reference, self, universe, variable, normal, TypeMismatch } from '@/code/check/kernel'
...
const book: Book = {
  ...
  Equal: {
    type: universe,
    value: lambda((a) =>
      lambda((b) => pi(pi(universe, () => universe), (P) => pi(apply(P, a), () => apply(P, b)))),
    ),
  },
  ...
  Bool: {
    ...
    value: apply(
      ...
      lambda((s) =>
        pi(universe, (P) =>
          pi(pi(eq(ref('true'), s), () => P), () => pi(pi(eq(ref('false'), s), () => P), () => P)),
        ),
      ),
    ),
  },
  ...
  true: {
    type: ref('Bool'),
    value: lambda(() => lambda((t) => lambda(() => apply(t, apply(ref('refl'), universe))))),
  },
  // false : Bool = lambda P. lambda t. lambda f. (f (refl *))
  false: {
    type: ref('Bool'),
    value: lambda(() => lambda(() => lambda((f) => apply(f, apply(ref('refl'), universe))))),
  },
  ...
  match: {
    type: pi(ref('Bool'), (b) =>
      pi(pi(ref('Bool'), () => universe), (P) =>
        pi(apply(P, ref('true')), () => pi(apply(P, ref('false')), () => apply(P, b))),
      ),
    ),
    value: lambda((b) =>
      lambda((P) =>
        lambda((t) =>
          lambda((f) =>
            apply(
              apply(apply(b, apply(P, b)), lambda((e) => apply(apply(e, P), t))),
  ...
  not: {
    ...
    value: lambda((b) =>
      apply(apply(apply(apply(ref('match'), b), lambda(() => ref('Bool'))), ref('false')), ref('true')),
    ),
  },
  // theorem : forall (b: Bool) (Equal (not (not b)) b)
  //         = lambda b. (match b lambda b.(Equal (not (not b)) b) (refl *) (refl *))
  theorem: {
    type: pi(ref('Bool'), (b) => eq(apply(ref('not'), apply(ref('not'), b)), b)),
    value: lambda((b) =>
      apply(
        apply(
          apply(apply(ref('match'), b), lambda((b2) => eq(apply(ref('not'), apply(ref('not'), b2)), b2))),
...
function main(): void {
  ...
  let caught = false
  try {
    normal(book, annotate(annotate(variable(0), reference('true')), reference('false')))
  } catch (error) {
