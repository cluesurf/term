...
export function millByHand(
    ...
    switch (keyword) {
      ...
      case 'fork': {
        ...
        if (variant !== 'case') {
          ...
          // the same inline condition the statement fork takes: `fork test, <cond>` writes it as a sibling of
          // `test` rather than in a `hook test` arm, and the hook loop below never sees it. See the statement
          // fork for the full account; this is the value-position copy of that path.
          const namedFlag = args
            .slice(1)
            .find(
              (n): n is GroupNode =>
                n.kind === 'group' && headName(n) === 'name',
            )
          const flagNode = namedFlag ? rest(namedFlag)[0] : undefined
          const flagName =
            flagNode?.kind === 'group' ? headName(flagNode) : undefined
          const inlineCond = args
            .slice(1)
            .find(
              (n): n is GroupNode =>
                n.kind === 'group' &&
          ...
          let pendingCond: Expression | undefined = flagName
            ? { form: 'variable', name: flagName, span }
            : inlineCond
              ? toExpression(inlineCond, scope)
              : undefined
    ...
    for (let at = 0; at < nodes.length; at++) {
      ...
      switch (keyword) {
        ...
        case 'fork': {
          ...
          // `fork test, <cond>` writes the condition INLINE, as a sibling of `test`, instead of in a
          // `hook test` arm. It used to be skipped by the hook loop below, so the following `hook hold`
          // found no condition and fell back to an always-true branch: `fork test, call is-equal(a, b)`
          // compiled to `if (true)` with the `hook miss` branch dead. Sixteen stdlib sites were affected and
          // nothing reported it, because the program was still well typed and still built. Found by the
          // mint-bridge parity gate, 2026-08-31.
          //
          // `fork test, name <flag>` dispatches on a named boolean parameter. It is documented as a
          // compile-time dispatch and NOTHING implements the folding, so it was reaching the emitter as an
          // always-true branch: `replace` in native/kotlin/text.tree ignored its `all` flag and always
          // replaced every occurrence. Lowered here as an ordinary read of the flag, which is what it means at
          // run time and what any later specialization has to agree with.
          const namedFlag = rest(node)
            .slice(1)
            .find(
              (n): n is GroupNode =>
                n.kind === 'group' && headName(n) === 'name',
  ...
  function buildHookCommand(group: GroupNode): DockRoute {
    ...
    // nested `hook`s are subcommands. The FIRST child is the command's own name and never a subcommand: for
    // `hook hook` (the zone console's clipboard command) it was counted as one, producing a phantom
    // subcommand with an empty path. Found by the mint-bridge parity gate, 2026-08-31.
    const children = rest(group)
      .slice(1)
      .filter(
        (n): n is GroupNode =>
