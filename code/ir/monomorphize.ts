...
function matchType(declared: Type | undefined, actual: Type | undefined, generics: Set<string>, subst: Map<string, Type>): void {
  if (!declared || !actual) return
  if (declared.kind === 'named' && generics.has(declared.name)) {
    subst.set(declared.name, actual)
    return
  }
  if (declared.kind === 'array' && actual.kind === 'array') matchType(declared.element, actual.element, generics, subst)
  ...
  } else if (declared.kind === 'function' && actual.kind === 'function') {
    declared.params.forEach((p, i) => matchType(p, actual.params[i], generics, subst))
    matchType(declared.result, actual.result, generics, subst)
  }
}

// replace generic names in a type with their concrete bindings
function substituteType(type: Type | undefined, subst: Map<string, Type>): Type | undefined {
  if (!type) return type
  switch (type.kind) {
    case 'named':
      return subst.get(type.name) ?? type
    case 'array':
      return { kind: 'array', element: substituteType(type.element, subst)! }
    case 'map':
      return { kind: 'map', key: substituteType(type.key, subst)!, value: substituteType(type.value, subst)! }
    case 'function':
      return { kind: 'function', params: type.params.map((p) => substituteType(p, subst)!), result: substituteType(type.result, subst)!, effects: type.effects }
    default:
      return type
  }
}
...
function typeKey(type: Type | undefined): string {
  ...
  switch (type.kind) {
    ...
    case 'function':
      return `fn_${type.params.map(typeKey).join('_')}_to_${typeKey(type.result)}`
    default:
      return type.kind
  }
}
...
function visitCalls(body: Array<Statement>, onCall: (node: Extract<Expression, { form: 'call' }>) => void): void {
...
export function monomorphize(program: Program): Program {
  ...
  for (const statement of program) if (statement.form === 'function' && statement.generics.length > 0) generics.set(statement.name, statement)
  ...
  function rewrite(body: Array<Statement>): void {
    visitCalls(body, (node) => {
      ...
      fn.params.forEach((param, i) => matchType(param.type, node.args[i]?.type, genericNames, subst))
      const mangled = `${fn.name}__${fn.generics.map((g) => typeKey(subst.get(g.name))).join('__')}`
      if (!specials.has(mangled)) {
        const spec = structuredClone(fn) as Fn
        spec.name = mangled
        spec.generics = []
        spec.params = spec.params.map((p) => ({ ...p, type: substituteType(p.type, subst) }))
  ...
  for (const statement of program) {
    if (statement.form === 'function' && statement.generics.length > 0) continue
