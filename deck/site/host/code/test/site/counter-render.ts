...
export function counter(host: View): number {
  const count = { value: 0, observers: [] }
  const root = { handle: { tag: "button", text: "", value: "", attributes: [], styles: [], classes: [], children: [], listeners: [], parent: { form: "none" } } }
  event(root, "click", () => {
  writeSignal(count, readSignal(count) + 1)
})
  mount(root, () => (dynamic(() => (readSignal(count)))))
  mount(host, () => (root))
}
