...
import { binary, indexGet, memberGet } from '@/code/engine/engine'
import {
  type Value, UNIT, integer as mkInt, float as mkFloat, boolean as mkBool, string as mkStr,
  truthy as isTruthy, display as show, keyOf,
...
export const int = (v: bigint | number): Value => mkInt(typeof v === 'bigint' ? v : BigInt(v))
...
export const array = (items: Value[]): Value => ({ form: 'array', value: Arr.fromArray(items) })
...
export const and = (a: Value, right: () => Value): Value => (isTruthy(a) ? right() : a)
export const or = (a: Value, right: () => Value): Value => (isTruthy(a) ? a : right())
...
export const member = (c: Value, name: string): Value => memberGet(c, name)
export function setIndex(c: Value, i: Value, v: Value): Value {
  if (c.form === 'array') return { form: 'array', value: Arr.set(c.value, Number((i as { value: { value: bigint } }).value.value), v) }
  if (c.form === 'map') { Mp.set(c.value, keyOf(i), v); return c }
  throw new Error(`cannot index-assign a ${c.form}`)
}
export function setMember(c: Value, name: string, v: Value): Value {
  if (c.form === 'map') { Mp.set(c.value, `s:${name}`, v); return c }
  throw new Error(`cannot set member on a ${c.form}`)
}

// built-ins (bound to plain names in the emitted preamble)
export const print = (...args: Value[]): Value => { console.log(args.map(show).join(' ')); return UNIT }
...
export const toInt = (v: Value): Value => {
  ...
  if (v.form === 'string') { const s = show(v); return int(BigInt(s)) }
