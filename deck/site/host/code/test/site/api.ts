...
export function maybeUnwrapOr<T>(self: Maybe, fallback: T): T {
  if (self.form === "some") {
    return self.value
  } else if (self.form === "none") {
    return fallback
  }
}
...
export function makeApi(): Server {
  const routes = []
  listPush(routes, { method: "GET", path: "/health", handle: (request: Request, params: Map<number, number>) => ({ status: 200, body: "ok" }) })
  listPush(routes, { method: "GET", path: "/users/:id", handle: (request: Request, params: Map<number, number>) => ({ status: 200, body: maybeUnwrapOr(hashGet(params, "id"), "none") }) })
  listPush(routes, { method: "GET", path: "/files/**", handle: (request: Request, params: Map<number, number>) => ({ status: 200, body: maybeUnwrapOr(hashGet(params, "rest"), "none") }) })
  return routeServer(routes)
}
