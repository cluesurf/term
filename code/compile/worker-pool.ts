...
import { processDef, buildDefContext } from '@/code/compile/def-pass'
import type { DefContext, DefOutput } from '@/code/compile/def-pass'
...
// the seed package root (two levels up from code/compile/), whose tsconfig carries the `@/*` path mapping
...
export class WorkerPool {
  ...
  private onMessage(entry: WorkerEntry, message: unknown): void {
    const msg = message as
      | { type: 'result'; id: number; name: string; typed: FunctionDef; diagnostics: Array<Diagnostic>; ts: string; threadId: number }
