...
import { buildIndex, referenceAt, occurrencesOf, scopeAt, callAt } from '@/code/server/symbols'
...
type PositionParams = { textDocument: { uri: string }; position: LspPosition }
type RenameParams = PositionParams & { newName: string }

// LSP SymbolKind / CompletionItemKind numeric codes for each of our kinds
const SYMBOL_KIND: Record<SymbolKind, number> = { function: 12, type: 10, variant: 22, trait: 11, parameter: 13, local: 13 }
const COMPLETION_KIND: Record<SymbolKind, number> = { function: 3, type: 7, variant: 20, trait: 8, parameter: 6, local: 6 }
// the keywords offered in completion (the four-letter Seed vocabulary), each a plain keyword item
const KEYWORDS = ['task', 'take', 'send', 'back', 'call', 'read', 'save', 'host', 'make', 'bind', 'form', 'case', 'head', 'link', 'fork', 'hook', 'walk', 'load', 'find', 'mark', 'text', 'wave', 'like', 'note', 'hold', 'dock', 'mask', 'wear', 'suit']
  ...
  dispatch(message: Message): Array<Message> {
    switch (message.method) {
      ...
      case 'textDocument/didOpen': {
        ...
        return [this.refresh(params.textDocument.uri, params.textDocument.text ?? '')]
      }
      case 'textDocument/didChange': {
        const params = message.params as ChangeParams
        return [this.refresh(params.textDocument.uri, params.contentChanges[params.contentChanges.length - 1]?.text ?? '')]
      }
      ...
      case 'textDocument/definition': {
        ...
        return [respond(message, def ? { uri: params.textDocument.uri, range: toRange(def.span) } : null)]
      }
      case 'textDocument/references': {
        ...
        return [respond(message, spans.map((span) => ({ uri: params.textDocument.uri, range: toRange(span) })))]
      }
      case 'textDocument/rename': {
        ...
        const edits = occurrencesOf(index!, ref.name).map((span) => ({ range: toRange(span), newText: params.newName }))
        return [respond(message, { changes: { [params.textDocument.uri]: edits } })]
      }
      case 'textDocument/documentSymbol': {
        ...
        const symbols = index
          ? [...index.definitions.values()]
              ...
              .map((d) => ({ name: d.name, detail: d.detail, kind: SYMBOL_KIND[d.kind], range: toRange(d.span), selectionRange: toRange(d.span) }))
          : []
      ...
      case 'textDocument/completion': {
        ...
        const fromScope = index
          ? scopeAt(index, params.position).map((d) => ({ label: d.name, kind: COMPLETION_KIND[d.kind], detail: d.detail }))
          : []
        const keywords = KEYWORDS.map((label) => ({ label, kind: 14 }))
        return [respond(message, { isIncomplete: false, items: [...fromScope, ...keywords] })]
      }
      case 'textDocument/signatureHelp': {
        ...
        const label = `${call.name}(${sig.params.map((p) => `${p.name}: ${p.type}`).join(', ')}) -> ${sig.result}`
        return [
          respond(message, {
            signatures: [{ label, parameters: sig.params.map((p) => ({ label: `${p.name}: ${p.type}` })) }],
            activeSignature: 0,
            activeParameter: Math.min(call.activeParam, Math.max(0, sig.params.length - 1)),
          }),
        ]
      }
      default:
