...
const eddsa = (() => {
  ...
  return {
    ...
    sign: async (privateKey: Uint8Array, message: Uint8Array): Promise<Uint8Array> => {
      const key = await subtle().importKey('pkcs8', concat(PKCS8_PREFIX, privateKey), { name: 'Ed25519' }, false, ['sign'])
