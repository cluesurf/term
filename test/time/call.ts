...
// `.seed/time/<name>.json`, and the save -> compare round-trip with regression gating -- without measuring anything.
...
async function readBaseline(
  ...
  const file = path.join(root, '.seed', 'time', `${name}.json`)
...
async function main(): Promise<void> {
  ...
  // comparison layer can later read back.
  {
    ...
    if (trap.calls.length === 0) {
      ...
      const history = path.join(root, '.seed', 'time', 'history')
  ...
  {
    const root = await makeProject(source)
    const dir = path.join(root, '.seed', 'time')
