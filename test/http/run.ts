...
async function main(): Promise<void> {
  ...
  try {
    ...
    ok(':param route binds the param', user.status === 200 && user.body === '42', JSON.stringify(user))
    const user2 = M.handleRequest(server, { method: 'GET', path: '/users/abc', body: '' })
    ok(':param matches a different value', user2.body === 'abc', JSON.stringify(user2))
    const files = M.handleRequest(server, { method: 'GET', path: '/files/a/b/c.txt', body: '' })
    ok('** catch-all binds the joined rest', files.status === 200 && files.body === 'a/b/c.txt', JSON.stringify(files))
    ...
    ok('static beats param at same depth', M.handleRequest(server, { method: 'GET', path: '/health', body: '' }).body === 'ok')
  } finally {
