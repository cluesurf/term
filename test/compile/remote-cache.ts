...
async function main(): Promise<void> {
  ...
  const server = startRemoteCacheServer({ storeDir: store, port: PORT, token: TOKEN })
  ...
  const built = compile({ file: 'a.tree', text: SOURCE }, { cache: cacheA })
  ok('project A compiles + writes its local cache', built.ok && fs.readdirSync(path.join(projectA, 'mill')).length > 0)

  const pushed = await pushRemoteCache(projectA, endpoint, TOKEN)
  ok('push uploads A local artifacts to the remote', pushed > 0, `pushed ${pushed}`)
  ok('a second push is a no-op (remote already has them)', (await pushRemoteCache(projectA, endpoint, TOKEN)) === 0)

  // project B: empty cache, pull from the remote to warm it
  const pulled = await pullRemoteCache(projectB, endpoint, TOKEN)
  ok('pull downloads the artifacts into B', pulled > 0 && pulled === pushed, `pulled ${pulled}`)
  ...
  const coldB = compile({ file: 'a.tree', text: SOURCE }, { cache: cacheB })
  ok('B cold compile hits the warmed remote cache', cacheB.misses === 0 && cacheB.diskHits > 0, `misses ${cacheB.misses} diskHits ${cacheB.diskHits}`)
  ok('B output equals A output', coldB.ok && built.ok && coldB.typescript === built.typescript)

  // auth: a wrong token is rejected
  const unauthorized = await fetch(`${endpoint}/index`, { headers: { authorization: 'Bearer wrong' } })
  ok('the remote rejects a bad token', unauthorized.status === 401, `${unauthorized.status}`)
