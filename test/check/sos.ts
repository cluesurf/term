...
ok(
  '(a-b)^2 >= 0 expanded',
  nonNegativeDifference(
    add(sub(mul(a, a), mul(i(2), mul(a, b))), mul(b, b)),
    i(0),
  ),
)
ok(
  'n^2 - 2n + 1 >= 0',
  nonNegativeDifference(add(sub(mul(n, n), mul(i(2), n)), i(1)), i(0)),
)
ok(
  'a^2+b^2+c^2 >= ab+bc+ca',
  nonNegativeDifference(
    add(add(mul(a, a), mul(b, b)), mul(c, c)),
    add(add(mul(a, b), mul(b, c)), mul(c, a)),
  ),
)
ok(
  '2a^2+2b^2 >= (a+b)^2',
  nonNegativeDifference(
    add(mul(i(2), mul(a, a)), mul(i(2), mul(b, b))),
    mul(add(a, b), add(a, b)),
  ),
)
...
ok(
  'NOT: a^2 - 2b^2 >= 0',
  !nonNegativeDifference(sub(mul(a, a), mul(i(2), mul(b, b))), i(0)),
)
ok('NOT: a^2 >= 1', !nonNegativeDifference(mul(a, a), i(1)))
ok(
  'NOT: a >= 0 (linear, not a square form)',
  !nonNegativeDifference(a, i(0)),
)
ok(
  'NOT: 2ab >= a^2+2b^2',
  !nonNegativeDifference(
    mul(i(2), mul(a, b)),
    add(mul(a, a), mul(i(2), mul(b, b))),
  ),
)
