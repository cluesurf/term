...
function main(): void {
  ...
  {
    const program = lower(`zone counter\n  take label, like text\n  zone div\n    read app/count\n`)
    ...
    ok('element nests a read child', element?.form === 'element' && element.children[0]?.form === 'read')
  }
  ...
  // a server route lowers to a `dock` with HTTP method handlers
  {
    ...
    ok('dock collects methods', JSON.stringify(dock?.route.methods.map((m) => m.name)) === '["get","post"]')
    ok('method collects its call', dock?.route.methods[0]?.calls[0]?.name === 'list-users')
  }

  // a CLI command lowers to a dock with options and a handler
  {
    const program = lower(`dock make\n  take name\n    like text\n    need true\n  call make-deck\n    bind name, read name\n`)
    const dock = program.find((s): s is Extract<Statement, { form: 'dock' }> => s.form === 'dock')
    ok('CLI command lowers to a dock', dock?.route.path === 'make')
    ok('dock collects options', dock?.route.takes[0]?.name === 'name')
    ok('option requiredness is captured', dock?.route.takes[0]?.required === true)
    ok('dock collects the handler call', dock?.route.calls[0]?.name === 'make-deck')
  }
  ...
  // a client route lowers to a dock that renders a zone component
  {
    ...
    const dock = program.find((s): s is Extract<Statement, { form: 'dock' }> => s.form === 'dock')
    ok('client route maps to a component', dock?.route.component?.name === 'counter')
    ok('component prop is captured', dock?.route.component?.props[0]?.name === 'label')
  }

  // `dock load` still lowers to a native FFI binding (not a route)
  {
    const program = lower(`dock load\n  load <node:fs>, name fs\n`)
    ok('dock load stays native FFI', program.some((s) => s.form === 'native'))
    ok('dock load is not a route', !program.some((s) => s.form === 'dock'))
  }
