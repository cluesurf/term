...
function makeStubElement(tag: string): any {
  return {
    tagName: tag, children: [] as any[], attributes: {} as Record<string, string>, listeners: {} as Record<string, Array<() => void>>, textContent: '', value: '',
...
async function main(): Promise<void> {
  ...
  ok('blog compiles against the browser DOM (bind)', result.ok, result.ok ? '' : JSON.stringify(result.diagnostics.slice(0, 4)))
  if (!result.ok) { console.log(`\nblog: ${pass} pass, ${fail} fail`); return }

  ok('emits native createElement/appendChild', /\.createElement\(/.test(result.typescript) && /\.appendChild\(/.test(result.typescript))
  ok('does not shadow the document global', !/const document = \w/.test(result.typescript))

  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seed-blog-'))
  fs.writeFileSync(path.join(dir, 'app.ts'), result.typescript)
  fs.writeFileSync(path.join(dir, 'entry.ts'), `import { makeApp } from './app'\nmakeApp({ handle: document.body })\n`)
  ...
  ok('renders an Add button', addButton?.tagName === 'button' && textOf(addButton) === 'Add post')
  ok('post list starts empty', posts?.tagName === 'div' && posts.children.length === 0)
  ...
  ok('post renders heading + body', post1?.children?.[0]?.tagName === 'h2' && post1?.children?.[1]?.tagName === 'p')
  ok('post heading is the title', textOf(post1.children[0]) === 'First Post', JSON.stringify(textOf(post1?.children?.[0])))
  ok('post body is the content', textOf(post1.children[1]) === 'Hello world', JSON.stringify(textOf(post1?.children?.[1])))
  ok('inputs cleared after adding', titleInput.value === '' && bodyInput.value === '')
  ...
  ok('first post unchanged', textOf(posts.children[0].children[0]) === 'First Post')
  ok('second post rendered', textOf(posts.children[1].children[0]) === 'Second Post' && textOf(posts.children[1].children[1]) === 'More text')

  // emit runnable artifacts
  const web = path.join(DECK, 'site.tree/host/web-blog')
  ...
  ok('emits browser artifacts (index.html + app.js)', fs.existsSync(path.join(web, 'index.html')))
