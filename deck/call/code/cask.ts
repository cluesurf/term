...
function cargoManifest(crate: string): string {
  ...
  const dependencies = own && existsSync(own) ? (readFileSync(own, 'utf8').split('[dependencies]')[1] ?? '').split(/\n\[/)[0] ?? '' : ''
  ...
  return [
    ...
    'windows = { version = "0.58", features = ["Win32_Foundation", "Win32_UI_WindowsAndMessaging", "Win32_System_LibraryLoader", "Win32_System_Com", "Win32_System_Com_StructuredStorage", "Win32_UI_Shell", "Win32_Storage_FileSystem", "Win32_Graphics_Gdi"] }',
    'webview2-com = "0.33"',
    '',
    '[profile.release]',
    'opt-level = 3',
...
export function buildRustProgram({
  ...
  writeFileSync(path.join(project, 'Cargo.toml'), cargoManifest(crate))
