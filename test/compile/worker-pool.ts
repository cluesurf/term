...
import { buildGlobalScope } from '@/code/check/resolve'
import { emitProgramParallel, WorkerPool } from '@/code/compile/worker-pool'
...
async function main(): Promise<void> {
  ...
  ok('inline build emits every function', [0, 1, 2, 7].every(i => inline.ts.includes(`function f${i}`)), inline.ts.slice(0, 80))
  ok('inline build has no diagnostics on a clean program', inline.diagnostics.length === 0, JSON.stringify(inline.diagnostics.map(d => d.name)))
  ok('inline build uses no worker threads', inline.threads.length === 0)

  // pooled: real worker threads
  const pool = new WorkerPool(3)
  try {
    const pooled = await emitProgramParallel(program, buildGlobalScope, pool)

    ok('pooled build produces byte-identical TypeScript (differential guarantee)', pooled.ts === inline.ts)
    ok('pooled build produces identical diagnostics', JSON.stringify(pooled.diagnostics) === JSON.stringify(inline.diagnostics))
    ok('pooled build actually ran on multiple worker threads', pooled.threads.length >= 2, `threads: ${pooled.threads.join(',')}`)
    ...
    const pooled2 = await emitProgramParallel(program2, buildGlobalScope, pool)
    const inline2 = await emitProgramParallel(program2, buildGlobalScope)
    ok('a second pooled build (warm workers, new revision) matches inline', pooled2.ts === inline2.ts)
  } finally {
    await pool.close()
  }
