// Remote compile cache (Tier 5). The local `.seed/cache` is content-addressed, so it shares across machines / CI with
...
export async function pullRemoteCache(
  ...
  const have = new Set(localEntries(cacheDir).map(e => `${e.kind}/${e.key}`))
  let pulled = 0
...
export async function pushRemoteCache(
  ...
  const remote = indexResponse.ok
    ? ((await indexResponse.json()) as Array<{ kind: Kind; key: string }>)
    : []
  const have = new Set(remote.map(e => `${e.kind}/${e.key}`))
  let pushed = 0
  for (const { kind, key } of localEntries(cacheDir)) {
    if (have.has(`${kind}/${key}`)) continue
    const body = readFileSync(path.join(cacheDir, kind, `${key}.json`), 'utf8')
    const response = await fetch(`${endpoint}/${kind}/${key}`, {
      method: 'PUT',
...
export function startRemoteCacheServer(options: {
  ...
  const authed = (context: { req: { header: (n: string) => string | undefined } }): boolean =>
  ...
  app.put('/:kind/:key', async context => {
    ...
    writeFileSync(path.join(dir, `${key}.json`), await context.req.text())
