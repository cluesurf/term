  ...
  try {
    const M = (await import(file)) as {
      createElement: (tag: string) => {
        handle: { children: Array<{ handle: { text: string } }> }
      }
    ...
    ok(
      'component mounts a reactive text node',
      mounted === '0',
      JSON.stringify(mounted),
    )
    M.writeSignal(count, '42')
    ok(
      'writing the signal updates the exact DOM node',
      root.handle.children[0]?.handle.text === '42',
      JSON.stringify(root.handle.children[0]?.handle.text),
    )
  } finally {
