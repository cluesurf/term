// Structural equality and stability for expressions and statements, shared across the compiler: the lint rules that
// ask whether two subexpressions are "the same" (no-self-comparison, no-duplicate-branch-condition,
// no-self-assignment, no-identical-branches) and the IR simplifier's boolean idempotence / absorption.
//
// `expressionsEqual` is purely structural: same shape, same literals, same names, ignoring spans / types / bindings.
// `isStable` is true when an expression reads no call or await, so evaluating it twice yields the same value. A caller
