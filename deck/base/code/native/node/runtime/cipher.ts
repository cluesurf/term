...
const cipher = (() => {
  ...
  return {
    encrypt: async (key: Uint8Array, nonce: Uint8Array, plain: Uint8Array): Promise<Uint8Array> => {
      const cryptoKey = await importKey(key)
      const sealed = await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv: nonce }, cryptoKey, plain)
      return new Uint8Array(sealed)
    },
    decrypt: async (key: Uint8Array, nonce: Uint8Array, sealed: Uint8Array): Promise<Uint8Array> => {
      const cryptoKey = await importKey(key)
      const opened = await globalThis.crypto.subtle.decrypt({ name: 'AES-GCM', iv: nonce }, cryptoKey, sealed)
