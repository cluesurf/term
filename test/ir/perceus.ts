...
let pass = 0
let fail = 0
function expect(name: string, got: Array<Inst>, want: Array<string>): void {
...
const make = (ctor: string, ...args: Array<string>): Inst['value' & keyof Inst] => ({ kind: 'make', ctor, args } as never)
...
function main(): void {
  ...
  expect(
    'used once: no dup',
    perceus(['x'], [
      { op: 'let', name: 'a', value: { kind: 'make', ctor: 'box', args: ['x'] } },
      { op: 'return', name: 'a' },
    ]),
  ...
  expect(
    ...
    perceus(['p'], [
      { op: 'let', name: 'q', value: { kind: 'call', fn: 'use', args: ['p'] } },
      { op: 'let', name: 'r', value: { kind: 'call', fn: 'combine', args: ['q', 'p'] } },
      { op: 'return', name: 'r' },
    ]),
  ...
  expect(
    ...
    perceus(['x', 'y', 'z'], [
      { op: 'let', name: 'a', value: { kind: 'make', ctor: 'box', args: ['x'] } },
      { op: 'let', name: 'b', value: { kind: 'make', ctor: 'pair', args: ['y', 'z'] } },
      { op: 'return', name: 'b' },
    ]),
    ['let a = make box(x)', 'drop a', 'let b = make pair(y, z)', 'return b'],
  )
  ...
  expect(
    ...
    perceus(['list', 'head'], [
      { op: 'let', name: 'pair', value: { kind: 'make', ctor: 'cons', args: ['list', 'head'] } },
      // `pair` is never used -> dropped; the next make of arity 2 reuses it
      { op: 'let', name: 'next', value: { kind: 'make', ctor: 'cons', args: ['list', 'head'] } },
      { op: 'return', name: 'next' },
    ]),
    // pair is dead -> drop pair, then next (arity 2) reuses pair's cell. `list`/`head` used twice -> dup'd.
    ['dup list', 'dup head', 'let pair = make cons(list, head)', 'let next = make cons(list, head) reuse pair', 'return next'],
  )
    ...
    perceusControl(['x'], [
      {
        ...
        else: [{ op: 'let', name: 'z', value: { kind: 'lit' } }, { op: 'return', name: 'z' }],
      },
    ]),
  ...
  expect(
    ...
    perceusControl(['x'], [
      { op: 'if', cond: 'c', then: [{ op: 'return', name: 'x' }], else: [{ op: 'return', name: 'x' }] },
    ]),
