...
export function zonk(type: Type, sub: Substitution): Type {
  ...
  if (t.kind === 'map')
    return { kind: 'map', key: zonk(t.key, sub), value: zonk(t.value, sub) }
  if (t.kind === 'function')
    return {
      kind: 'function',
  ...
  if (t.kind === 'named' && t.args)
    return { kind: 'named', name: t.name, args: t.args.map(a => zonk(a, sub)) }
  return t
}
...
export function zonkGeneric(
  ...
  if (r.kind === 'array')
    return { kind: 'array', element: zonkGeneric(r.element, names, sub) }
  if (r.kind === 'map')
    return {
      kind: 'map',
